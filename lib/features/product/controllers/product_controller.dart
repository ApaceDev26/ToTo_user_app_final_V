...
class ProductController extends GetxController implements GetxService {
  ...
  Future<Product?> getProductDetails(int id, CartModel? cart, {bool isCampaign = false}) async {
    try {
      final fetched = await productServiceInterface.getProductDetails(id: id, isCampaign: isCampaign);
      if(fetched != null) {
        fetched.variations ??= [];
        fetched.addOns ??= [];
        _product = fetched;
      ...
      debugPrint('getProductDetails failed: $e\n$s');
      return null;
    }
  }
    ...
    if(_popularProductList == null || reload || fromRecall) {
      try {
        ...
        if(dataSource == DataSourceEnum.local) {
          ...
          // Await client so failures are handled; do not clear existing list first.
          await getPopularProductList(false, type, false, dataSource: DataSourceEnum.client, fromRecall: true);
        } else {
          popularProductList = await productServiceInterface.getPopularProductList(type: type, source: DataSourceEnum.client);
          if(popularProductList != null) {
            _preparePopularProductList(popularProductList);
          } else if(_popularProductList == null) {
            // End shimmer when the API returns nothing / errors.
            _popularProductList = [];
            update();
          }
        }
