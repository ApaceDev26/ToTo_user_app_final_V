...
class ProductService implements ProductServiceInterface {
  ...
  List<List<bool?>> initializeSelectedVariation(List<Variation>? variations) {
    ...
    if (variations != null) {
      for (int index = 0; index < variations.length; index++) {
        ...
        final minCount = isMultiSelect ? (variation.min ?? 1) : 1;

        if (isMultiSelect) {
          // Multi-select: auto-select first minCount in-stock options
          int selected = 0;
          for (int i = 0; i < values.length; i++) {
            bool select = selected < minCount && _isVariationValueInStock(values[i]);
            if (select) selected++;
            selectedVariations[index].add(select);
          }
        } else {
          // Single-select: auto-select first in-stock option (fallback: first option)
          int firstInStock = -1;
          for (int i = 0; i < values.length; i++) {
            if (_isVariationValueInStock(values[i])) {
              firstInStock = i;
              break;
            }
          }
          ...
          for (int i = 0; i < values.length; i++) {
            selectedVariations[index].add(i == firstInStock);
          }
        }
      }
    }
  ...
  bool _isVariationValueInStock(VariationValue value) {
    if (value.stockType == 'unlimited') return true;
    if (value.currentStock == null) return true;
    return value.currentStock! > 0;
  }
  ...
  List<bool> initializeAddonActiveList(List<AddOns>? addOns) {
    List<bool> addOnActiveList = [];
    if (addOns == null) return addOnActiveList;
  ...
  List<int?> initializeAddonQuantityList(List<AddOns>? addOns) {
    List<int?> addOnQtyList = [];
    if (addOns == null) return addOnQtyList;
    for (var addOn in addOns) {
      debugPrint('$addOn');
      addOnQtyList.add(1);
    }
