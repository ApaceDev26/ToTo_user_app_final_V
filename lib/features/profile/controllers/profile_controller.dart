...
class ProfileController extends GetxController implements GetxService {
  ...
  Future<bool> sendOtpForAccountDeletion() async {
    _isLoading = true;
    update();
    Response response = await profileServiceInterface.deleteUser();
    _isLoading = false;
    ...
    if (response.statusCode == 200) {
      // Check if OTP is required (response contains otp_required flag)
      if (response.body != null && response.body['otp_required'] == true) {
        return true; // OTP sent successfully
      } else {
        // Account deleted without OTP (shouldn't happen with new flow)
        return false;
      }
    } else {
      // Handle errors
      if (response.body != null && response.body['errors'] != null) {
        ...
        if (errors.isNotEmpty && errors[0]['code'] == 'on-going') {
          showCustomSnackBar(errors[0]['message'] ?? 'error_occurred'.tr);
  ...
  Future<bool> verifyOtpAndDeleteAccount(String otp) async {
    _isLoading = true;
    update();
    Response response = await profileServiceInterface.deleteUserWithOtp(otp);
    _isLoading = false;
    ...
    if (response.statusCode == 200) {
      ...
      if (response.body != null && response.body['errors'] != null) {
        ...
        showCustomSnackBar('error_occurred'.tr);
      }
      update();
      return false;
    }
