...
import 'package:toto_user/common/models/product_model.dart' as pv;
...
class CartHelper {
  ...
  static List<CartModel> formatOnlineCartToLocalCart({required List<OnlineCartModel> onlineCartModel}) {
    ...
    for (OnlineCartModel cart in onlineCartModel) {
      ...
      // For 'amount' discount: per-unit discount, so total discount = discount * quantity
      double discountedPrice;
      if (discountType == 'amount') {
        double totalDiscount = (discount ?? 0) * quantity;
        discountedPrice = price - totalDiscount;
      } else {
        discountedPrice = PriceConverter.convertWithDiscount(price, discount, discountType)!;
      }
