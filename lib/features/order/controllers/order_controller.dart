...
import 'package:toto_user/features/order/domain/models/delivery_log_model.dart';
import 'package:toto_user/features/order/domain/models/order_cancellation_body.dart';
...
import 'package:toto_user/features/order/domain/models/pause_log_model.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  void setBottomSheetExpanded(bool expanded) {
    _bottomSheetExpanded = expanded;
    update();
  }
  ...
  VoidCallback? _collapseBottomSheetCallback;

  void setCollapseBottomSheetCallback(VoidCallback? callback) {
    _collapseBottomSheetCallback = callback;
  }

  void collapseBottomSheet() {
    if (_collapseBottomSheetCallback != null) {
      _collapseBottomSheetCallback!();
    }
  }

  Future<void> getRunningOrders(int offset,
      {bool notify = true, int limit = 100, bool silentRefresh = false}) async {
    if (offset == 1) {
      // For silent refresh, keep existing list to avoid flickering
      // But still allow refresh by removing from offset list if it exists
      if (silentRefresh && _runningOffsetList.contains(offset)) {
        _runningOffsetList.remove(offset);
  ...
  Future<bool> cancelOrder(int? orderID, String? cancelReason) async {
    ...
    if (responseModel.isSuccess) {
      _runningOrderList?.removeWhere((order) => order.id == orderID);
      _showCancelled = true;
      showCustomSnackBar(responseModel.message, isError: false);
    }
    update();
