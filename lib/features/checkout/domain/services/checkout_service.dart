...
class CheckoutService implements CheckoutServiceInterface {
  ...
  Future<double?> getDistanceInKM(LatLng originLatLng, LatLng destinationLatLng, {bool isDuration = false}) async {
    ...
    try {
      if (response.statusCode == 200 && response.body is Map && response.body['distanceMeters'] != null) {
        if(isDuration){
          final String duration = response.body['duration'] as String? ?? '0s';
          double parsedDuration = parseDuration(duration);
          distance = parsedDuration / 3600;
        }else{
          final double distanceMater = (response.body['distanceMeters'] as num).toDouble();
          distance = distanceMater / 1000;
        }
      } else if (response.statusCode == 200 && response.statusText == 'OK') {
        if(isDuration){
