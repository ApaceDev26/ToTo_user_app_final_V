...
import 'package:toto_user/features/chat/domain/models/message_model.dart';
import 'package:toto_user/features/chat/domain/services/chat_service_interface.dart';
import 'package:toto_user/features/chat/enums/user_type_enum.dart';
import 'package:toto_user/helper/date_converter.dart';
import 'package:toto_user/helper/image_size_checker.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> _pollForNewMessages() async {
    // Don't poll if loading or if message model/notification body is null
    if (_messageModel == null || _currentNotificationBody == null) return;

    // Skip polling if currently loading (but don't stop the timer)
    if (_isLoading) return;

    Response? response;
    if (_currentNotificationBody == null ||
        _currentNotificationBody!.adminId != null) {
      response =
          await chatServiceInterface.getMessages(1, 0, UserType.admin, null);
    } else if (_currentNotificationBody!.restaurantId != null) {
      response = await chatServiceInterface.getMessages(
          1,
          _currentNotificationBody!.restaurantId,
          UserType.vendor,
          _currentConversationId);
    } else if (_currentNotificationBody!.deliverymanId != null) {
      // Always use conversation_id if available for more accurate polling
      response = await chatServiceInterface.getMessages(
          1,
          _currentNotificationBody!.deliverymanId,
          UserType.delivery_man,
          _currentConversationId);
    } else {
      // No valid notification body - stop polling
      return;
    }

    if (response != null && response.statusCode == 200) {
      // Check if messages field exists and is a list (even if empty)
      if (response.body['messages'] != null &&
        ...
        if (newMessageModel.messages != null) {
          ...
          if (newMessages.isNotEmpty) {
            ...
            _sortMessage(_currentNotificationBody!.adminId);
            update();
          }
        }
      }
  ...
  Future<void> getMessages(int offset, NotificationBodyModel? notificationBody,
    ...
    if (response != null &&
      ...
      if (offset == 1) {
        ...
        // Ensure polling continues with updated message model
        if (_isPollingActive &&
            (_currentNotificationBody != notificationBody ||
                _currentConversationId != conversationID)) {
          startMessagePolling(notificationBody, user, conversationID);
        }
      } else {
          ...
          if (_getFileSizeFromPlatformFileToDouble(element) >
            ...
            if (objFile.length < AppConstants.maxLimitOfTotalFileSent) {
              if ((await _getMultipleFileSizeFromPlatformFiles(objFile) +
                ...
                double fileSize =
                    await ImageSize.getImageSizeFromXFile(element.xFile);
  ...
  Future<Response?> sendMessage(
    ...
    if (messageModel != null) {
      ...
      _sortMessage(notificationBody!.adminId);
      // Ensure polling continues after sending message - restart it to use latest message model
      startMessagePolling(notificationBody, null, conversationID);
      Future.delayed(const Duration(seconds: 2), () {
        getMessages(1, notificationBody, null, conversationID).then((_) {
          // Restart polling after getMessages completes to ensure it uses the latest model
          if (_isPollingActive) {
            startMessagePolling(notificationBody, null, conversationID);
          }
        });
