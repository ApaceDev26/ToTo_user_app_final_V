...
import 'package:toto_user/features/wallet/domain/models/fund_bonus_model.dart';
import 'package:toto_user/features/wallet/domain/models/wallet_model.dart';
...
class WalletRepository implements WalletRepositoryInterface{
  ...
  Future<Response> addFundToWallet(double amount, String paymentMethod) async {
    ...
    // Mobile must send platform=app so gateway returns callback with flag=...
    // Web uses host wallet URL; app uses admin wallet path (detected in payment redirect).
    final String callback = isWeb
        ? '$protocol//$hostname${RouteHelper.wallet}'
        : '${AppConstants.baseUrl}${RouteHelper.wallet}';

    return await apiClient.postData(AppConstants.addFundUri, {
      "amount": amount,
      "payment_method": paymentMethod,
      "payment_platform": isWeb ? 'web' : 'app',
      "callback": callback,
    });
  }
