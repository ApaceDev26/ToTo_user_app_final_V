...
import 'package:toto_user/features/checkout/widgets/bottom_section_widget.dart';
import 'package:toto_user/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:toto_user/features/checkout/widgets/order_place_button.dart';
import 'package:toto_user/features/checkout/widgets/top_section_widget.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    ...
    // Trigger in-app messages for checkout screen
    Future.delayed(const Duration(milliseconds: 800), () {
      InAppMessagingHelper.triggerForScreen('checkout');
  ...
  Future<void> initCall() async {
    ...
    checkoutController.setRestaurantDetails(
    ...
    checkoutController.tipController.text =
        checkoutController.selectedTips != -1
            ? AppConstants.tips[checkoutController.selectedTips]
            : '';
    ...
    return Scaffold(
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => ConfirmationDialogWidget(
                  icon: Images.support,
                  title: 'clear_cart'.tr,
                  description: 'are_you_sure_to_clear_cart'.tr,
                  onYesPressed: () async {
                    ...
                    Get.offNamed(
                        RouteHelper.getInitialRoute()); // Navigate to homepage
                  },
                ),
              );
            },
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn()
          ? GetBuilder<CartController>(builder: (cartController) {
              // Check if cart is empty and navigate to homepage
              if (cartController.cartList.isEmpty &&
                  !Get.currentRoute.contains(RouteHelper.payment) &&
                  !Get.currentRoute.contains(RouteHelper.orderSuccess)) {
                ...
                return (checkoutController.distance != null &&
                    ...
                    ? GetBuilder<LocationController>(
                        ...
                        if (checkoutController.restaurant != null) {
                          ...
                          tomorrowClosed =
                              checkoutController.isRestaurantClosed(
                        ...
                        return GetBuilder<CouponController>(
                          ...
                          // Use cartController.cartList when fromCart is true, otherwise use _cartList
                          List<CartModel>? currentCartList = widget.fromCart
                              ? cartController.cartList
                              : _cartList;

                          double price = currentCartList != null &&
                                  currentCartList.isNotEmpty
                              ? _calculatePrice(currentCartList)
                              : 0;
                          double addOnsPrice = currentCartList != null &&
                                  currentCartList.isNotEmpty
                              ? _calculateAddonsPrice(currentCartList)
                              : 0;

                          double? discount = _calculateDiscountPrice(
                          ...
                          double subTotal =
                              _calculateSubTotal(price, addOnsPrice);
                          ...
                          Future.delayed(const Duration(milliseconds: 100), () {
                            if (checkoutController.isFirstTime ||
                              ...
                              // Use currentCartList for PlaceOrderBodyModel creation
                              List<CartModel>? cartListForOrder =
                                  widget.fromCart
                                      ? cartController.cartList
                                      : _cartList;
                              if (cartListForOrder == null ||
                                  cartListForOrder.isEmpty) {
                                return;
                              }
                              for (int index = 0;
                                  index < cartListForOrder.length;
                                  index++) {
                                CartModel cart = cartListForOrder[index];
                                  ...
                                  for (int i = 0;
                                    ...
                                    if (cart.variations![i].contains(true)) {
                                      variations.add(OrderVariation(
                                          name:
                                              cart.product!.variations![i].name,
                                          values:
                              ...
                              PlaceOrderBodyModel placeOrderBody =
                                ...
                                couponCode: (Get.find<CouponController>()
                                    ...
                                    : null,
                                restaurantId: currentCartList != null &&
                                        currentCartList.isNotEmpty
                                    ? currentCartList[0].product?.restaurantId
                                    : null,
                                discountAmount: discount,
                                cutlery: Get.find<CartController>().addCutlery
                                    ? 1
                                    : 0,
                                ...
                                extraPackagingAmount:
                                    Get.find<CartController>().needExtraPackage
                                        ? checkoutController
                                            .restaurant!.extraPackagingAmount
                                        : 0,
                          ...
                          // Preference +15 only when delivery is not free
                          if (checkoutController.orderType == 'delivery' &&
                              deliveryCharge > 0 &&
                              checkoutController.deliveryPreferenceCharge > 0) {
                            deliveryCharge = PriceConverter.toFixed(
                                deliveryCharge +
                                    checkoutController.deliveryPreferenceCharge);
                          }

                          double extraPackagingCharge =
                              _calculateExtraPackagingCharge(
                                  checkoutController);
                          print(
                              'Extra Packaging Charge: $extraPackagingCharge');
                          ...
                          if (_payableAmount !=
                              ...
                              checkoutController.distance != null &&
                              isLoggedIn &&
                              Get.find<HomeController>().cashBackOfferList !=
                                  null &&
                                                    ...
                                                    Expanded(
                                                      ...
                                                      child:
                                                        ...
                                                        referralDiscount:
                                                            referralDiscount,
                                                        extraPackagingAmount:
                                                            extraPackagingCharge, /*extraDiscount: extraDiscount*/
                                                      ),
                                                    )
                                                  ]),
                                            )
                                        ...
                                        children: [
                                          Padding(
                                            ...
                                            child: Row(children: [
                                              ...
                                              (checkoutController.taxIncluded ==
                                                      1)
                                                  ? Text(' ${'vat_tax_inc'.tr}',
                                                      style:
    ...
    if (restaurant.selfDeliverySystem == 0 &&
        (configModel?.adminFreeDelivery?.status == true &&
            (configModel?.adminFreeDelivery?.type != null &&
                configModel?.adminFreeDelivery?.type ==
                    'free_delivery_by_specific_criteria') &&
            (configModel!.adminFreeDelivery!.freeDeliveryDistance! > 0 &&
                configModel.adminFreeDelivery!.freeDeliveryDistance! >=
                    checkoutController.distance!))) {
      deliveryCharge = 0;
      charge = 0;
    }
...
/*    if(restaurant.selfDeliverySystem == 0 && Get.find<SplashController>().configModel!.freeDeliveryDistance != null && Get.find<SplashController>().configModel!.freeDeliveryDistance! >= checkoutController.distance!){
    ...
    // Check if free delivery for new customer is enabled
    if (configModel?.adminFreeDelivery?.freeDeliveryForNewCustomer == true &&
        Get.find<AuthController>().isLoggedIn() &&
        Get.find<ProfileController>().userInfoModel != null &&
        (Get.find<ProfileController>().userInfoModel!.orderCount == null ||
            Get.find<ProfileController>().userInfoModel!.orderCount == 0)) {
      deliveryCharge = 0;
      charge = 0;
    }
