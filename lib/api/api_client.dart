...
class ApiClient extends GetxService {
  ...
  Response handleResponse(http.Response response, String uri, bool handleError,
    ...
    _logApiResponse(response0.statusCode, uri, response0.body);
    if (handleError) {
      if (response0.statusCode == 200) {
        return response0;
      } else {
  ...
  static const int _logBodyLimit = 180;

  void _logApiResponse(int? status, String uri, dynamic body) {
    if (!kDebugMode) return;
    if (status == 200) return;
    debugPrint('API [$status] $uri ${_clip(body)}');
  }

  String _clip(dynamic value) {
    final s = value.toString().replaceAll(RegExp(r'\s+'), ' ');
    if (s.length <= _logBodyLimit) return s;
    return '${s.substring(0, _logBodyLimit)}… (${s.length}c)';
  }
}
